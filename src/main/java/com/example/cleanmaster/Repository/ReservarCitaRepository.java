...
public interface ReservarCitaRepository extends JpaRepository<ReservarCitaEntities, Integer> {
    @Query("SELECT r FROM ReservarCitaEntities r WHERE r.idCliente = :idCliente AND r.fecha BETWEEN :lunes AND :domingo and r.finalizadaReserva = false")
    List<ReservarCitaEntities> getReservarPorClienteEntreLunesYDomingo(Integer idCliente, LocalDate lunes, LocalDate domingo);

    @Query("SELECT r FROM ReservarCitaEntities r WHERE r.idEmpleado = :idEmpleado AND r.fecha BETWEEN :lunes AND :domingo and r.finalizadaReserva = false")
